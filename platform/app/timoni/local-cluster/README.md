...
A [timoni.sh](http://timoni.sh) module for deploying local-cluster to Kubernetes clusters.
...
```shell
timoni -n default apply local-cluster oci://<container-registry-url>
```
...
```shell
timoni -n default delete local-cluster
```
