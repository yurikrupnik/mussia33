        ...
        Ok(result) => {
            if result.rows_affected() > 0 {
                res.status_code(StatusCode::OK)
                    .render(Json(format!("Deleted item with ID: {}", id)))
            } else {
                res.status_code(StatusCode::OK)
                    .render(Json(format!("Item with ID: {} not found", id)))
            }
        }
        Err(e) => res
            .status_code(StatusCode::INTERNAL_SERVER_ERROR)
            .render(Json(e.to_string())),
    }
}
...
pub async fn get_user(_id: PathParam<String>, res: &mut Response) {
    let pool = db_pool();
    let query = &format!("SELECT * FROM {} where id = '{}'", User::COLLECTION, _id);
...
pub async fn drop_users(res: &mut Response) {
    ...
    match result {
        Ok(payload) => res
            ...
            .render(Json(payload.rows_affected())),
        Err(e) => res
            .status_code(StatusCode::INTERNAL_SERVER_ERROR)
            .render(Json(e.to_string())),
    }
