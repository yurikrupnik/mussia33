...
pub async fn delete_author(id: PathParam<String>, res: &mut Response) {
    ...
    let collection: Collection<Author> = mongo_client
        .database(Author::DB)
    ...
    if result.deleted_count == 1 {
        ...
        res.status_code(StatusCode::NOT_FOUND)
            .render(Json("product with specified ID not found!"))
    }
}
...
pub async fn get_author(id: PathParam<String>, res: &mut Response) {
    ...
    if _id.is_empty() || _id.len() != 24 {
        res.status_code(StatusCode::BAD_REQUEST)
            .render(Json(format!("Invalid ID: {}", &_id)))
    };
    let mongo_client = mongo_pool();
    ...
    match result {
        ...
        Ok(None) => res
            ...
            .render(Json(format!("No product found with id {obj_id}"))),
        Err(err) => res
...
pub async fn update_author(body: JsonBody<UpdateDto>, id: PathParam<String>, res: &mut Response) {
    ...
    if id.is_empty() || id.len() != 24 {
        res.status_code(StatusCode::BAD_REQUEST)
            .render(Json(format!("Invalid ID: {}", id)))
    };
    let obj_id = ObjectId::parse_str(id).unwrap();
    let filter = doc! {"_id": obj_id};
    let mongo_client = mongo_pool();
...
pub async fn create_author(body: JsonBody<CreateDto>, res: &mut Response) {
    ...
    match response {
        ...
        Ok(None) => res
            .status_code(StatusCode::NOT_FOUND)
            .render(Json(format!("No user found with id {new_id}"))),
        Err(err) => res
            .status_code(StatusCode::INTERNAL_SERVER_ERROR)
            .render(Json(err.to_string())),
    }
