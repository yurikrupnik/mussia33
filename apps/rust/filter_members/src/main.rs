...
struct Args {
  ...
  #[arg(short, long, value_name = "actix_app")]
  app: String,

  /// Number of times to greet
  #[arg(short, long, value_name = "tmp/Cargo.toml")]
  path: String,
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
  ...
  let members = parsed_toml.get_mut("workspace")
  ...
  let has_apps = members.iter()
    .any(|x| x.as_str().map(|s| s.contains("apps"))
    .unwrap_or(false));
  ...
  println!("{}", format!("Filtered TOML file saved as '{}'", path).blue());
  Ok(())
}
